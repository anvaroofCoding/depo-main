...
import { Badge, Button, Empty, Image, Input, Space, Table, Tooltip } from 'antd'
...
export default function Depos() {
	...
	const columns = [
...

		{
			...
			key: 'guruhi',
			width: 150,
			filters: [...new Set(filteredData?.map(item => item.guruhi))].map(g => ({
				text: g,
				value: g,
			})),
		...
		},
		{
			...
			key: 'turi',
			width: 150,
			filters: [...new Set(filteredData?.map(item => item.turi))].map(g => ({
				text: g,
				value: g,
			})),
